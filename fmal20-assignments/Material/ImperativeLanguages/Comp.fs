    ...
    let rec addv decs varEnv funEnv = 
        ...
        | dec::decs  -> 
            ...
            | Vardec (typ, var) ->
                ...
                let varEnv', funEnv', code' = addv decs varEnv funEnv
                varEnv', funEnv', code @ code'
            | Fundec (tyOpt, f, xs, body) ->
                addv decs varEnv ((f, (newLabel(), tyOpt, xs)) :: funEnv)
    addv topdecs ([], 0) []
...
let rec cStmt stmt (varEnv : varEnv) (funEnv : funEnv) : instr list = 
    ...
    | Block stmts -> 
        let rec loop stmts varEnv =
            ...
            | []     -> snd varEnv, []
            | stmt::stmts -> 
                let varEnv', code = cStmtOrDec stmt varEnv funEnv
                let fdepth, code' = loop stmts varEnv'
                fdepth, code @ code'
        let (fdepth, code) = loop stmts varEnv
        code @ [INCSP (snd varEnv - fdepth)]
...
and cExpr (e : expr) (varEnv : varEnv) (funEnv : funEnv) : instr list = 
    ...
    match e with
    | Access acc     ->                             // a - means rvalue
         cAccess acc varEnv funEnv
         @ [LDI]

    | Addr acc       ->                             // &a - means lvalue
         cAccess acc varEnv funEnv   
